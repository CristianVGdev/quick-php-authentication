...
if(isset($_SESSION['sub'])) {
  ...
  echo '<p>' . htmlspecialchars($_SESSION['username'], ENT_QUOTES, 'UTF-8') . '</p>'; // It is sanitized to prevent XSS
...
if(!isset($_GET['code'])) {
  ...
  if(isset($_GET['error'])) {
    die('Authorization server returned an error: ' . htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8'));
  }
...
function http($url, $params = false) {
  ...
  // The resource is closed to prevent memory leaks
  if (is_array($params) && !empty($params)) {
      curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($params));
  }
  $result = curl_exec($ch);
  curl_close($ch); // The resource is closed to prevent memory leaks
